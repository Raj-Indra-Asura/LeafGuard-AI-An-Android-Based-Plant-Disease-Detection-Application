...
The repository is **Kotlin-first**: the primary app is `android-app-kotlin/` (Kotlin,
Google's recommended language for new Android apps). The syllabus topic "Java for
Android development" is fully satisfied by the complete Java twin in `android-app/` —
every mapping in the table above is implemented identically in both tracks, so evidence
may be collected from either. The two apps share the same features, behavior, backend,
and model. See `docs/parallel-track/README.md` and `docs/JAVA_VS_KOTLIN.md`.
